"""
Men's collection page for WERBEAUTY.
Product listing with filters and search for men's products. 
"""
...
from utils.product_loader import load_men_products, filter_products, search_products
from utils.helpers import highlight_text


def render():
    """
    Render the men's collection page. 
    """
    ...
    render_animated_header(
        ...
        subtitle="Premium grooming essentials for the modern gentleman",
        gender="men"
    )
    ...
    with col_filters:
        filters = render_filters_panel(gender="men")
    ...
    with col_products:
        ...
        if search_query:
            ...
            st.markdown(f"""
            <div style="
                background: linear-gradient(135deg, rgba(10, 26, 63, 0.1), rgba(192, 195, 200, 0.1));
                padding: 1rem 1.5rem;
                border-radius: 12px;
                margin-bottom: 1.5rem;
                border-left: 4px solid #0A1A3F;
            ">
        ...
        # Product grid
        if products:
            render_product_grid(products, columns=3, key_prefix="men")
        else:
...
def render_search_section():
    ...
    search = st.text_input(
        ...
        placeholder="Search beard care, grooming, perfumes.. .",
...
def render_empty_results():
    ...
    if st.button("🔄 Reset All Filters", key="reset_men_filters", use_container_width=True):
